...
resource auditJob 'Microsoft.App/jobs@2024-03-01' = {
  ...
  properties: {
    ...
    template: {
      containers: [
        {
          name: 'mysql-gemini-config-auditor'
...
SELECT 'channel',
       id,
       name,
       JSON_UNQUOTE(JSON_EXTRACT(`key`, '$.project_id')) AS project_id,
       models,
       other
  FROM channels
...
SELECT 'option',
       `key`,
       CASE
         WHEN `key` = 'GroupRatio' THEN value
         WHEN `key` = 'GroupModelRatio' THEN value
         ELSE JSON_OBJECT(
           'gemini-3.5-flash', JSON_EXTRACT(value, '$."gemini-3.5-flash"'),
           'gemini-3.6-flash', JSON_EXTRACT(value, '$."gemini-3.6-flash"'),
           'gemini-3.7-flash', JSON_EXTRACT(value, '$."gemini-3.7-flash"')
         )
       END AS model_values
  FROM options
 WHERE `key` IN (
...
SELECT 'ability', model, `group`, enabled, priority, weight, tag
  FROM abilities
 WHERE channel_id = 2
...
'''
  ...
  tags: {
    ...
    component: 'database-gemini-config-audit'
    managedBy: 'bicep'
  }
}
